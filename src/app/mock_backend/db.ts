export const db = {
  etag: 'DuHzAJ-eQIiCIp7p4ldoVcVAOeY/1bBuvoRES2RbSsm8zrrFEToKhaE',
  kind: 'youtube#searchListResponse',
  nextPageToken: 'CDIQAA',
  items: [
    {
      'kind': 'youtube#searchResult',
      'etag': '\'DuHzAJ-eQIiCIp7p4ldoVcVAOeY/QtJ4MlYKdN_zTBjfY3xY6mn7ZRg\'',
      'id': {
        'kind': 'youtube#video',
        'videoId': '3fumBcKC6RE'
      },
      'snippet': {
        'publishedAt': '2011-05-12T20:01:31.000Z',
        'channelId': 'UCEOhcOACopL42xyOBIv1ekg',
        'title': 'Lil Wayne - John (Explicit) ft. Rick Ross',
        'description': 'Music video by Lil Wayne performing John. (C) 2011 Cash Money Records Inc.',
        'thumbnails': {
          'default': {
            'url': 'https://i.ytimg.com/vi/3fumBcKC6RE/default.jpg',
            'width': 120,
            'height': 90
          },
          'medium': {
            'url': 'https://i.ytimg.com/vi/3fumBcKC6RE/mqdefault.jpg',
            'width': 320,
            'height': 180
          },
          'high': {
            'url': 'https://i.ytimg.com/vi/3fumBcKC6RE/hqdefault.jpg',
            'width': 480,
            'height': 360
          }
        },
        'channelTitle': 'LilWayneVEVO',
        'liveBroadcastContent': 'none'
      }
    },
    {
      'kind': 'youtube#searchResult',
      'etag': '\'DuHzAJ-eQIiCIp7p4ldoVcVAOeY/pRlb6OnS5jGoiGdwuOT_OXsdwjA\'',
      'id': {
        'kind': 'youtube#video',
        'videoId': 'nG2pEffLEJo'
      },
      'snippet': {
        'publishedAt': '2018-06-04T06:30:00.000Z',
        'channelId': 'UC3XTzVzaHQEd30rQbuvCtTQ',
        'title': 'Guardianship: Last Week Tonight with John Oliver (HBO)',
        'description': 'It is alarmingly easy for a total stranger to gain total control over the life of a senior citizen.',
        'thumbnails': {
          'default': {
            'url': 'https://i.ytimg.com/vi/nG2pEffLEJo/default.jpg',
            'width': 120,
            'height': 90
          },
          'medium': {
            'url': 'https://i.ytimg.com/vi/nG2pEffLEJo/mqdefault.jpg',
            'width': 320,
            'height': 180
          },
          'high': {
            'url': 'https://i.ytimg.com/vi/nG2pEffLEJo/hqdefault.jpg',
            'width': 480,
            'height': 360
          }
        },
        'channelTitle': 'LastWeekTonight',
        'liveBroadcastContent': 'none'
      }
    }
  ]
};
